...
output goes. Native had nothing, so reading a sculpting session meant tailing
`chisel-debug.log` in another window and matching timestamps by eye. The instruments this
project keeps adding are only useful if the person sculpting can see them.

`~` toggles it, PgUp/PgDn scroll, and it works during confirm dialogs — which is often
exactly when you want to read what just happened.

It **captures rather than hooks**: stdout and stderr are redirected into a pipe at startup
and drained once per frame, so every existing printf in the codebase appears without a
single call site changing. Drained bytes are written straight back out to the real stdout,
so `chisel-debug.log` receives everything exactly as before — verified against a file
redirect, not assumed. stdout is forced line-buffered, since a pipe is block-buffered by
default and whole strokes of output would otherwise sit unflushed.

Lines are coloured by tag: `[arena]`/`[cull]`/crash-guards red, `[stage]`/`[dirty]` cyan,
other bracketed tags yellow. The red set is exactly what `chisel-debug.sh` greps for when
it decides whether a run was clean.

The pipe is enlarged to 1 MB rather than given a non-blocking write end: a full pipe would
then drop output silently, and a debug console that lies by omission is worse than none.
The cost of capture is that output reaches the log when the frame drains it rather than the
instant it is printed, so a hard crash can lose up to one frame of text.

Compiled out on web, where `shell.html` already owns the key. Windows uses `PeekNamedPipe`
