// mask_smooth.wgsl
// Lockstep sibling of shaders/glsl/mask_smooth.comp. Mask-smooth: the smooth
// gesture while the mask brush is active. Blends each in-radius vertex's MASK
...
// touching geometry. Same dual-anchor mirror as mask_paint; no mask-shield
...
//   6 dirty (read_write) 12 mask (read_write)   63 params UBO
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
    ...
    let nv  = clamp(mix(mask_buf[v], avg, a), 0.0, 1.0);
    if (nv == mask_buf[v]) {
        return;
    }
