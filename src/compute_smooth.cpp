...
// --- Clay wall-fill (tangential, anisotropy-gated relax) ---------------------------

namespace {
// 16-byte std140 block, byte-identical to clay_relax.{comp,wgsl}'s Params.
struct ClayRelaxParamsGPU {
    uint32_t dirty_count; float strength; float aniso_lo; float fill_bias;
};
static_assert(sizeof(ClayRelaxParamsGPU) == 16, "clay_relax Params UBO must be 16 bytes");
}

bool ComputeState::init_clay_relax() {
    if (!supported) return false;
    const gpu::BindEntry layout[] = {
        { BIND_POSITIONS,        gpu::Bind::StorageReadWrite, 0 },
        ...
        { BIND_DIRTY_VERTS,      gpu::Bind::StorageRead,      0 },
        { BIND_MASK,             gpu::Bind::StorageRead,      0 },
        { BIND_PARAMS,           gpu::Bind::Uniform,          sizeof(ClayRelaxParamsGPU) },
    };
    clay_relax_pipeline = gpu::create_compute_pipeline(gpu_dev,
                              gpu::embedded_shader("clay_relax"), layout, 8);
    if (!clay_relax_pipeline.handle) {
        std::printf("[compute] clay_relax pipeline failed to compile\n");
        return false;
    }
    clay_relax_ubo = gpu::create_buffer(gpu_dev, nullptr, sizeof(ClayRelaxParamsGPU), gpu::Usage::Uniform);
    std::printf("[compute] clay_relax pipeline compiled (gpu:: seam)\n");
    return true;
}

void ComputeState::dispatch_clay_relax(const uint32_t* vert_ids, uint32_t count,
    ...
    if (!clay_relax_pipeline.handle || count == 0 || !mask_ssbo.handle) return;

    // Reuse the shared dirty-vert id buffer (stroke_smooth / compute_normals). Grow-only.
    if (!dirty_verts_ssbo.handle || count > dirty_verts_capacity) {
        uint32_t alloc_count = std::max(count, 4096u);
        gpu::release_buffer(dirty_verts_ssbo);
        dirty_verts_ssbo = gpu::create_buffer(gpu_dev, nullptr,
    ...
    ClayRelaxParamsGPU u = {};
    u.dirty_count = count;
    u.strength = strength;
    u.aniso_lo = aniso_lo;
    u.fill_bias = fill_bias;
    gpu::write_buffer(gpu_dev, clay_relax_ubo, 0, &u, sizeof(u));
    ...
    const gpu::BindBufferEntry bg[] = {
        ...
        { BIND_DIRTY_VERTS,      &dirty_verts_ssbo,      dirty_verts_ssbo.size },
        { BIND_MASK,             &mask_ssbo, mask_ssbo.size },
        { BIND_PARAMS,           &clay_relax_ubo, sizeof(ClayRelaxParamsGPU) },
    };

    // In-place RW like autosmooth: each host-looped iteration reads the previous
    // pass's positions. A fresh bind group per iter keeps the seam happy.
    if (iters < 1) iters = 1;
    for (int it = 0; it < iters; it++) {
        gpu::BindGroup grp = gpu::create_bind_group(gpu_dev, clay_relax_pipeline, bg, 8);
        gpu::ComputeBatch b = gpu::begin_compute(gpu_dev);
        gpu::dispatch(b, clay_relax_pipeline, grp, (count + 255u) / 256u);
        gpu::submit(b);
        gpu::release_bind_group(grp);
        gpu::barrier(gpu_dev);
    }
